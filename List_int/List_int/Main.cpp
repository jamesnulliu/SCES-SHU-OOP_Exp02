...
int main()
{
    ...
    while (true)
    {
        ...
        case '1':
        case 'p':
        case 'P':	cout << "第一种格式输出" << endl;
            ShowList(head);
            cout << "\n第二种格式输出" << endl;
            cout << head << endl;							break;
        case '2':
        case 'i':
        case 'I':	cout << "插入一个结点到链表首结点前。请输入数据域数据(int): ";
            cin >> x;
            Insert(head, x);								break;
        case '3':
        case 'a':
        case 'A':	cout << "追加一个结点(至尾结点后)。请输入数据域数据(int): ";
            cin >> x;
            Append(head, x);								break;
        case '4':
        case 's':
        case 'S':	cout << "根据数据域数据查询。请输入待查询的数据(int): ";
            cin >> x;
            Find(head, x);									break;
        case '5':
        case 'w':
        case 'W':	n = Save(filename, head);
            cout << "将 " << n << " 个结点的数据写入 "
